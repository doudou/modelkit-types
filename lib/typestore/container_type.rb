module TypeStore
    ...
    class ContainerType < IndirectType
        ...
        # TypeStore instances pointing to elements should be invalidated after
        # certain operations
        #
        ...
        def handle_container_invalidation
            ...
            if invalidated? 
                ...
                TypeStore.debug { "invalidating all elements in #{raw_to_s}" }
                invalidate_children
            elsif @elements.size > self.size
                TypeStore.debug { "invalidating #{@elements.size - self.size} trailing elements in #{raw_to_s}" }
        ...
        module ConvertToRuby
            def __element_to_ruby(value)
                TypeStore.to_ruby(value, element_t)
            end
        end
        module ConvertFromRuby
            def __element_from_ruby(value)
                TypeStore.from_ruby(value, element_t)
            end
        end
