...
module TypeStore
    module CXX
        ...
        # A converted from the output of GCC-XML to the TypeStore's own XML format
        class GCCXMLLoader
            ...
            # Given a full TypeStore type name, returns a [name, id] pair where +name+
            ...
            def resolve_type_definition(xmlnode)
                ...
                if kind == "Struct" || kind == "Class"
                    ...
                    if xmlnode['name'] =~ /^\._(\d+)$/ # this represents anonymous enums
                        return ignore(xmlnode, "ignoring anonymous enumeration, as they can't be represented in TypeStore")
