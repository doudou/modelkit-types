module TypeStore
    ...
    convert_from_ruby TrueClass, '/bool' do |value, typestore_type|
        TypeStore.from_ruby(1, typestore_type)
    end
    convert_from_ruby FalseClass, '/bool' do |value, typestore_type|
        TypeStore.from_ruby(0, typestore_type)
    end
    convert_to_ruby '/bool' do |value|
        TypeStore.to_ruby(value, INT_BOOL_T) != 0
    end
    ...
    specialize '/std/string' do
        ...
        def concat(other_string)
            if other_string.respond_to?(:to_str)
                super(TypeStore.from_ruby(other_string, self.class))
            else super
            end
        end
    end
    specialize '/std/vector<>' do
        include TypeStore::ContainerType::StdVector
    end
    ...
    if String.instance_methods.include? :ord
        convert_from_ruby String, CHAR_T.name do |value, typestore_type|
            ...
            TypeStore.from_ruby(value[0].ord, typestore_type)
        end
    else
        convert_from_ruby String, CHAR_T.name do |value, typestore_type|
            if value.size != 1
            ...
            TypeStore.from_ruby(value[0], typestore_type)
        end
    end
    convert_from_ruby String, "#{CHAR_T.name}[]" do |value, typestore_type|
