module TypeStore
	...
	def pretty_print(pp) # :nodoc:
        ...
        def each
            ...
            raw_each do |el|
                yield(TypeStore.to_ruby(el, element_t))
            end
        end
        ...
        def raw_set(index, value)
            if value.kind_of?(Type)
                ...
                # If +value+ is already a typestore value, just do a plain copy
        ...
        def [](index, range = nil)
            if range
                result = []
                range.times do |i|
                    result << TypeStore.to_ruby(raw_get(i + index), element_t)
                end
                result
            else
                TypeStore.to_ruby(raw_get(index), element_t)
            end
        end

        def []=(index, value)
            raw_set(index, TypeStore.from_ruby(value, element_t))
        end
