...
module ModelKit::Types
    module CXX
        ...
        # A converted from the output of GCC-XML to the ModelKit::Types's own XML format
        class GCCXMLLoader
            ...
            def resolve_type_definition(xmlnode)
                ...
                if kind == "Struct" || kind == "Class"
                    ...
                    if type_name == "/std/string"
                        # ModelKit::Types does not (yet) have a character type class.
                        ...
                        # This is known to ModelKit::Types as a container
                    ...
                    when "/bool"
                        ...
                        if !IGNORED_FUNDAMENTALS.any? { |matcher| matcher === name }
                            raise InternalError, "do not know what to do with fundamental type #{name}"
                        end
                    end
                    ...
                    if xmlnode['name'] =~ /^\._(\d+)$/ # this represents anonymous enums
                        return ignore(xmlnode, "ignoring anonymous enumeration, as they can't be represented in ModelKit::Types")
            ...
            def load(required_files, xml)
                ...
                # Add the standardized version of all C++ types we know the size
                # of. A.k.a. not bool or char
                [1, 2, 4, 8].each do |size|
                    registry.create_numeric("/int#{size * 8}_t", size: size, integer: true, unsigned: false)
                    registry.create_numeric("/uint#{size * 8}_t", size: size, integer: true, unsigned: true)
                end
                registry.create_numeric("/float", size: 4, integer: false)
                registry.create_numeric("/double", size: 8, integer: false)
